...
# 创建 logger
logger = get_logger('glm_4_1v_thinking_flash_demo')

# 使用官方示例代码，完全按照文档
client = ZhipuAiClient(api_key=os.environ.get('ZAI_API_KEY'))

logger.info("========================================")
logger.info("GLM-4.1V-Thinking-Flash Demo")
logger.info("========================================")

logger.info("Model: GLM-4.1V-Thinking-Flash")
...
logger.info("========================================")
logger.info("Response from GLM-4.1V-Thinking-Flash")
logger.info("========================================")

# 输出完整的响应对象
logger.info(f"Full Response Object: {response}")
logger.info(f"Response Type: {type(response)}")
logger.info(f"Response Dict: {response.model_dump() if hasattr(response, 'model_dump') else str(response)}")
...
if response.choices:
    ...
    logger.info(f"Message Dict: {message.model_dump() if hasattr(message, 'model_dump') else str(message)}")
